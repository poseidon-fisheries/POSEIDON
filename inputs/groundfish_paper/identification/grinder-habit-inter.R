...
valid_areas<-
  unique(testlog %>% filter(year>=1) %>% filter(choice=="yes") %>% select(option)) 
...
fishing<-mlogit.data(as.data.frame(testlog),
                     ...
                     chid.var="chid")
#original:
reg<-mlogit(formula=choice~distance+habit+
              revenue+`Dover Sole_cpue`+
              Sablefish_cpue-1,data=fishing)
