...
public class SpeciesSpecificDiscardRatesFactory<S extends Scope>
    ...
    @Singular("discardRateBySpeciesKey")
    private Map<String, Double> discardRatesBySpeciesKey;
    ...
    protected SpeciesSpecificDiscardRates newInstance(final S scope) {
        final var rates = SpeciesSpecificRateFactorySupport.buildRatesByKey(
            species.get(scope),
            discardRatesBySpeciesKey,
            "discard rate"
        );
        return new SpeciesSpecificDiscardRates(rates);
    }

    public static <S extends Scope> SpeciesSpecificDiscardRatesFactory<S> fromFile(
        ...
        return new SpeciesSpecificDiscardRatesFactory<>(
            species,
            rates
        );
    }
