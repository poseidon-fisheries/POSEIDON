...
class MutableDoubleGridFromAllocatorFactoryTest {

    @Test
    void buildsGridFromAllocatorAndPredicateThenAllowsMutation() {
        final ModelGrid modelGrid = ModelGrid.create(2, 2, new Envelope(0, 2, 0, 2));
        final MutableDoubleGridFromAllocatorFactory<Scope> factory =
            new MutableDoubleGridFromAllocatorFactory<>(
                scope -> modelGrid,
        ...
        grid.setValue(new Int2D(0, 0), 42.0);
        assertThat(grid.getValue(new Int2D(0, 0))).isEqualTo(42.0);
    }
}
