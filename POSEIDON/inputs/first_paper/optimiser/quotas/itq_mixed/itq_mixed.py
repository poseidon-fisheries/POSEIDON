...
def build_input(filename, red, blue
    ...
    quotas = "0:" + str(float(red) / 300.0) + ",1:" + str(float(blue) / 300.0)
    print(quotas)
    data["Abstract"]["regulation"] = dict()
    data["Abstract"]["regulation"]["Multi-ITQ by List"] = dict()
    data["Abstract"]["regulation"]["Multi-ITQ by List"]["yearlyQuotaMaps"] = quotas
    data["Abstract"]["regulation"]["Multi-ITQ by List"]["allowMultipleTrades"] = False
    #data["Abstract"]["regulation"]["Multi-ITQ by List"]["minimumQuotaTraded"] = int(max(min(100, float(red) / 300.0, float(blue) / 300.0), 1))
...
def main(job_id, params):
    ...
    experiment_name = "itq_mixed_laptop" + str(job_id)
