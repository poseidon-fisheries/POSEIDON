...
/**
 * A simple planner where given a budget of time you can spend out: 1 - Draw the next action to take
 * from random distribution 2 - Use a generator to turn that action into a planned action (or a set
 * of possible choices) 3 - If you have multiple choices (as for example with setting on FADs) pick
 * the one whose profit/distance(centroid) is maximum 4 - Add that planned action in your path so
 * that it's the cheapest insertion possible 5 - Keep doing this until you run out of budget
 * <br>
...
public class DrawThenCheapestInsertionPlanner implements FisherStartable {
    ...
    /**
     * mapping that for each action returns the planning module giving us one candidate action to
     * take
     */
    final private Map<ActionType, PlanningModule> planningModules;

    /**
     * Actions of types listed here will be added to the plan first, before preferences are taken
     * into consideration. The first use case for this is forcing fishers to deploy FADs until they
     * reach their active-FADs limits.
     */
    ...
    /**
     * adds the path to the current plan and returns the hours it takes (duration + movement);
     * returns NaN if we go over budget! As a <b>side effect</b> it will <b>add the action to the
     * plan</b> if it is within budget! <br>
     * <p>
     ...
     * @param insertActionInPlanIfHoursAllowIt if true, the action is added to the plan with the
     *                                         cheapest possible insert. when this is set to false
     *                                         this method only computes the hypothetical hourly
     *                                         costs but does not insert the action in the given
     *                                         plan
     * @return the hours we consumed adding the action to the plan (or NAN if there are not enough
     * hours)
     */
    static Optional<Double> cheapestInsert(
        ...
        assert (actionToAddToPath.getLocation() != null) : "Action " +
            actionToAddToPath +
            " has no path!";
        ...
        // for each possible insertion point, adding point C between A and B has insertion cost
        // equal to d(A,C)+d(C,B)
        // -d(A,B);
        ...
        for (int index = 1; index < currentPlan.numberOfStepsInPath(); index++) {
            ...
            // it is never optimal to squeeze yourself between two actions that take place in the
            // same spot unless you
            ...
            final double firstSegment = map.distance(
                from.getLocation(),
                actionToAddToPath.getLocation()
            );
            if (firstSegment == 0) {
            ...
            final double secondSegment = map.distance(
                actionToAddToPath.getLocation(),
                to.getLocation()
            );
            ...
            final double insertionCost =
                (firstSegment + secondSegment - replacedSegment) /
                    speed; // turn insertion cost into time
            // tricky here because when you add the approximations from the curvature distance,
            // if you are going on a
            // straight line
            // in the projected map, you may actually be violating by tiny amounts the triangle
            // inequality constraint,
            // but we choose to ignore this
            // assert insertionCost >= -5 : "triangle inequality does not seem to hold here.
            // Bizarre " + insertionCost;
    ...
    private Plan makePlan(
        ...
        this.planningModules.values().forEach(module -> module.prepareForReplanning(
            fishState,
            fisher
        ));
        final Map<ActionType, AtomicLong> permittedActions = initPermittedActions();
        ...
        while (
            ...
            if (insertionCost.isPresent()) {
                ...
                permittedActions.get(nextActionType).decrementAndGet();
            } else {
    ...
    /**
     * Returns a map from action types to number of permitted actions, taking into action
     * preferences and regulations. Only action types with one or more permitted actions are
     * included in the map.
     */
    private Map<ActionType, AtomicLong> initPermittedActions() {
