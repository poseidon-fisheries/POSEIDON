...
public class LinearIncreaseInFishers {
    ...
    public static void main(final String[] args) {
        ...
        final Path container = Paths.get("runs", "entry-exit");
        container.toFile().mkdirs();
        FishStateUtilities.printCSVColumnsToFile(
            container.resolve("sample2.csv").toFile(),
    ...
    public static void slowInSlowOut(final String[] args) {
        final FishState state = new FishState(System.currentTimeMillis());
        Logger.getGlobal().setLevel(Level.OFF);
        ...
        container.toFile().mkdirs();
        FishStateUtilities.printCSVColumnsToFile(
            container.resolve("sample.csv").toFile(),
            state.getDailyDataSet().getColumn("Number of Fishers"),
            state.getDailyDataSet().getColumn("Species 0 Landings"),
            state.getDailyDataSet().getColumn("Biomass Species 0")
        );
