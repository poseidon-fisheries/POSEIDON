...
public class WriteMeristicsToFile {
    ...
    public void toWrite() throws Exception {
        ...
        final HashMap<String, MeristicsInput> inputs = new HashMap<>();
        ...
        final Path mainPath = Paths.get("inputs", "california", "biology");
        for (final Map.Entry<String, MeristicsInput> input : inputs.entrySet()) {
            final Path speciesDirectory = mainPath.resolve(input.getKey());
            speciesDirectory.toFile().mkdirs();
            final String dumped = yaml.dumpAsMap(input.getValue());
            final FileWriter writer = new FileWriter(speciesDirectory.resolve("meristics.yaml").toFile());
            writer.write(dumped);
            writer.flush();
            writer.close();
