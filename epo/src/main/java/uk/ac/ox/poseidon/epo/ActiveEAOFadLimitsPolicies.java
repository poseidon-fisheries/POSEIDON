...
import static uk.ac.ox.oxfish.fisher.purseseiner.regulations.DefaultEaoRegulations.ACTIVE_FAD_LIMITS;

public class ActiveEAOFadLimitsPolicies extends EAOPolicySupplier {

    private final int referenceYear;
    private final List<Double> proportionsOfCurrentLimits;

    ActiveEAOFadLimitsPolicies(
    ...
    @Override
    public List<Policy<EaoScenario<?>>> get() {
        return proportionsOfCurrentLimits
            .stream()
            .map(proportion ->
                new Policy<EaoScenario<?>>(
                    String.format(
