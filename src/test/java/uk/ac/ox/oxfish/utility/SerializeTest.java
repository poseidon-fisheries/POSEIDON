...
public class SerializeTest {
    ...
    public void randomSeedWorksCalifornia() throws Exception {
        Log.info("starts the california model, writes it down, reads it and runs it against the original copy. " +
                         "They should have the same results");
        ...
        assertEquals(state.getFishers().get(0).getLatestYearlyObservation(FisherYearlyTimeSeries.CASH_COLUMN),
                     state2.getFishers().get(0).getLatestYearlyObservation(FisherYearlyTimeSeries.CASH_COLUMN),
                     .001);

        assertEquals(state.getFishers().get(0).getDailyData().getLatestObservation(FisherYearlyTimeSeries.CASH_COLUMN),
